...
const mezclarColor = (
    ...
    const color1 = colorPincel
        .substring(1)
        .match(/\w\w/g)
        .map((valor) => parseInt(valor, 16));
    const color2 = colorFondo
        .slice(4, -1)
        .split(', ')
        .map((valor) => Number(valor));
    let colorFinal = '#';
    for (let i = 0; i < 3; i++) {
        colorFinal += Math.round(
            color2[i] + (color1[i] - color2[i]) * valorMezcla
        )
            .toString(16)
            .padStart(2, '0');
    }
...
const crearColor = (pixelDestino) => {
    ...
    if (acciones[1].checked && aerografo.checked)
        return mezclarColor('#f2efe6', pixelDestino, aerografoFlujo.value);
    if (acciones[1].checked) return '#f2efe6';
};

const crearPixeles = () => {
    tablero.style.height = '100%';
    let pixelesFilas = Math.floor(
        tablero.clientHeight / (tablero.clientWidth / reticulaDensidad.value)
    );
    ...
    for (let i = 1; i <= reticulaDensidad.value * pixelesFilas; i++) {
        ...
        pixel.addEventListener('mouseenter', (evento) => {
            ...
            if (dibujar)
                pixel.style.background = crearColor(
...
aerografoFlujo.addEventListener('input', () => {
    aerografoFlujoTexto.textContent = `${Math.round(
        aerografoFlujo.value * 100
    )}%`;
});
    ...
    switch (evento.code) {
        case 'KeyB':
            ...
            document.addEventListener(
                'mouseover',
                (evento) => {
                    evento.target.style.background = crearColor(
                        evento.target.style.background
                    );
                },
                { once: true }
        ...
        case 'KeyR':
            ...
            break;
        case 'KeyG':
            reticulaVisualizacion.checked = reticulaVisualizacion.checked
                ? false
                : true;
            verReticula();
            break;
